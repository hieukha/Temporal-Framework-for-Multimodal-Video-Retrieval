...
old_base_dir = "/dataset/AIC2024/pumkin_dataset/2/frames/autoshot/keyframes"
new_base_dir = "/dataset/AIC2024/pumkin_dataset/2/frames/autoshot"
...
for l_folder in os.listdir(old_base_dir):
    if l_folder.startswith("Videos_"):
        l_folder_name = l_folder.split("Videos_")[-1]  # Extract L folder name, e.g., L01
        print(l_folder_name)
        ...
        if os.path.isdir(old_l_folder_path):
            for v_folder in os.listdir(old_l_folder_path):
                ...
                new_video_dir = os.path.join(new_base_dir, f"Keyframes_{l_folder_name}", "keyframes", v_folder)
