...
from Extract_Feature.Class.SigLIP import SigLIP
...
print(f"✅ SigLIP model loaded in {model_load_time:.2f} seconds")

# Configuration
FEATURES_PATH = ["/dataset/AIC_2025/SIU_Sayan/autoshot/features_siglip"]
SPLIT_NAME = ["autoshot"]

# Initialize FAISS GPU connection with HIGH_ACCURACY for best results
print("🔄 Initializing FAISS GPU connection...")
start_time = time.time()
faiss_db = FAISS_GPU("siglip_v3", index_type="HIGH_ACCURACY")

# Create/use the siglip_v3 collection
print("🔗 Setting up 'siglip_v3' collection...")
# Check if index exists, if not create it
if not faiss_db.is_trained:
    print("FAISS index does not exist. Creating...")
    faiss_db.addDatabase("siglip_v3", 1152, FEATURES_PATH, SPLIT_NAME)
else:
...
if faiss_db.is_trained:
    dummy_query_path = "/workspace/competitions/AIC_2025/SIU_Sayan/Base/Cache/dummy_query_siglip.npy"
    if os.path.exists(dummy_query_path):
        ...
        dummy_query = np.random.random(1152).astype('float32')
...
def preprocessing_text(text: str) -> np.ndarray:
    """Preprocess text to feature vector using SigLIP model"""
    global model
...
def preprocessing_image(image: str) -> np.ndarray:
    """Preprocess image to feature vector using SigLIP model"""
    global model
...
app = FastAPI(
    title="AIC 2025 SIU Sayan Video Search API - SigLIP (FAISS GPU)",
    description="Advanced video search API using SigLIP model with FAISS GPU vector database",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc",
...
async def index():
    ...
    return {
        "message": "AIC 2025 SIU Sayan Video Search API - SigLIP (FAISS GPU)",
        "version": "1.0.0",
        "endpoints": {
        ...
        "description": "Advanced video search using SigLIP model with FAISS GPU acceleration",
        "index_type": faiss_db.index_type,
        "feature_size": 1152
    }
...
if __name__ == "__main__":
    ...
    print(f"🚀 Starting SigLIP FAISS GPU API server on port 8503...")
