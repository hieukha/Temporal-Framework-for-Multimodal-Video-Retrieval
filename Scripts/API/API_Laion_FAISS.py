...
from Extract_Feature.Class.Laion import LAION
from Vector_database.faiss_gpu import FAISS_GPU

# Initialize feature extraction model on CPU to save VRAM
print("🔄 Loading LAION CLIP model on CPU...")
start_time = time.time()
model = LAION(device="cpu")
model_load_time = time.time() - start_time
print(f"✅ LAION CLIP model loaded on CPU in {model_load_time:.2f} seconds")

# Configuration
FEATURES_PATH = ["/dataset/AIC_2025/SIU_Sayan/autoshot/features_laion"]
SPLIT_NAME = ["autoshot"]

# Initialize FAISS GPU connection with HIGH_ACCURACY for best results
print("🔄 Initializing FAISS GPU connection...")
start_time = time.time()
faiss_db = FAISS_GPU("laion_v3", index_type="HIGH_ACCURACY")

# Create/use the laion_v3 collection
print("🔗 Setting up 'laion_v3' collection...")
# Check if index exists, if not create it
if not faiss_db.is_trained:
    print("FAISS index does not exist. Creating...")
    faiss_db.addDatabase("laion_v3", 1024, FEATURES_PATH, SPLIT_NAME)
else:
...
if faiss_db.is_trained:
    dummy_query_path = "/workspace/competitions/AIC_2025/SIU_Sayan/Base/Cache/dummy_query.npy"
...
def preprocessing_text(text: str) -> np.ndarray:
    """Preprocess text to feature vector using LAION model"""
    global model
...
def preprocessing_image(image: str) -> np.ndarray:
    """Preprocess image to feature vector using LAION model"""
    global model
...
app = FastAPI(
    title="AIC 2025 SIU Sayan Video Search API - LAION (CPU Model + FAISS GPU)",
    description="Advanced video search API using LAION CLIP model on CPU with FAISS GPU vector database - Optimized for VRAM efficiency",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc"
)
...
async def index():
    ...
    return {
        "message": "AIC 2025 SIU Sayan Video Search API - LAION (CPU Model + FAISS GPU)",
        "version": "1.0.0",
        "endpoints": {
        ...
        "description": "Advanced video search using LAION CLIP model on CPU with FAISS GPU acceleration - Optimized for VRAM efficiency"
    }

if __name__ == "__main__":
    import uvicorn
    
    print(f"🚀 Starting FAISS GPU API server on port 8502...")
