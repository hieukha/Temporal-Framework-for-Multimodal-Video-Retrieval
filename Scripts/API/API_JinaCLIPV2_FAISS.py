...
from Extract_Feature.Class.JinaCLIPV2 import JinaCLIPV2
from Vector_database.faiss_gpu import FAISS_GPU

# Initialize feature extraction model
print("🔄 Loading JinaCLIPV2 model...")
start_time = time.time()
model = JinaCLIPV2()
model_load_time = time.time() - start_time
print(f"✅ JinaCLIPV2 model loaded in {model_load_time:.2f} seconds")

# Determine feature dimension dynamically
print("🔎 Determining feature dimension...")
try:
    probe_feats = model.get_text_features("hello world")
    if hasattr(probe_feats, 'cpu'):
        probe_feats = probe_feats.cpu().detach().numpy()
    probe_feats = np.array(probe_feats)
    if probe_feats.ndim == 2 and probe_feats.shape[0] == 1:
        FEATURE_DIM = int(probe_feats.shape[1])
    elif probe_feats.ndim == 1:
        FEATURE_DIM = int(probe_feats.shape[0])
    else:
        FEATURE_DIM = int(probe_feats.reshape(-1).shape[0])
except Exception:
    # Fallback if probing fails; common JinaCLIPv2 dim is 1024
    FEATURE_DIM = 1024
print(f"✅ Feature dimension: {FEATURE_DIM}")

# Configuration
FEATURES_PATH = ["/dataset/AIC_2025/SIU_Sayan/autoshot/features_jinaclipv2"]
SPLIT_NAME = ["autoshot"]

# Initialize FAISS GPU connection with HIGH_ACCURACY for best results
print("🔄 Initializing FAISS GPU connection...")
start_time = time.time()
faiss_db = FAISS_GPU("jinaclipv2_v3", index_type="HIGH_ACCURACY")

# Create/use the jinaclipv2_v3 collection
print("🔗 Setting up 'jinaclipv2_v3' collection...")
# Check if index exists, if not create it
if not faiss_db.is_trained:
    print("FAISS index does not exist. Creating...")
    faiss_db.addDatabase("jinaclipv2_v3", FEATURE_DIM, FEATURES_PATH, SPLIT_NAME)
else:
...
if faiss_db.is_trained:
    dummy_query_path = "/workspace/competitions/AIC_2025/SIU_Sayan/Base/Cache/dummy_query_jinaclipv2.npy"
    if os.path.exists(dummy_query_path):
        ...
        dummy_query = np.random.random(FEATURE_DIM).astype('float32')
...
def preprocessing_text(text: str) -> np.ndarray:
    """Preprocess text to feature vector using JinaCLIPV2 model"""
    global model
    ...
    if text_feat_arr.ndim == 2 and text_feat_arr.shape[0] == 1:
        text_feat_arr = text_feat_arr[0]
    elif text_feat_arr.ndim > 1:
        text_feat_arr = text_feat_arr.reshape(-1)
    
    return text_feat_arr.astype(np.float32)


def preprocessing_image(image: str) -> np.ndarray:
    """Preprocess image to feature vector using JinaCLIPV2 model"""
    global model
    ...
    # Handle both (D,) and (1, D) shapes
    if image_feat_arr.ndim == 2 and image_feat_arr.shape[0] == 1:
        image_feat_arr = image_feat_arr[0]
    elif image_feat_arr.ndim > 1:
        image_feat_arr = image_feat_arr.reshape(-1)
    
    return image_feat_arr.astype(np.float32)
...
app = FastAPI(
    title="AIC 2025 SIU Sayan Video Search API - JinaCLIPV2 (FAISS GPU)",
    description="Advanced video search API using JinaCLIPV2 model with FAISS GPU vector database",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc"
)
...
async def index():
    ...
    return {
        "message": "AIC 2025 SIU Sayan Video Search API - JinaCLIPV2 (FAISS GPU)",
        "version": "1.0.0",
        "endpoints": {
        ...
        "description": "Advanced video search using JinaCLIPV2 model with FAISS GPU acceleration",
        "index_type": faiss_db.index_type,
...
if __name__ == "__main__":
    ...
    print(f"🚀 Starting JinaCLIPV2 FAISS GPU API server on port 8505...")
