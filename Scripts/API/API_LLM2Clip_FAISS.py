...
from Extract_Feature.Class.LLM2Clip import LLM2Clip
from Vector_database.faiss_gpu_llm2clip import FAISS_GPU_LLM2

# Initialize feature extraction model
print("🔄 Loading LLM2Clip model...")
start_time = time.time()
model = LLM2Clip()
model_load_time = time.time() - start_time
print(f"✅ LLM2Clip model loaded in {model_load_time:.2f} seconds")

# Configuration
FEATURES_PATH = ["/dataset/AIC_2025/SIU_Sayan/autoshot/features_llm2clip"]
SPLIT_NAME = ["autoshot"]

# Initialize FAISS GPU connection with HIGH_ACCURACY for best results
print("🔄 Initializing FAISS GPU connection...")
start_time = time.time()
faiss_db = FAISS_GPU_LLM2("llm2clip_v3", index_type="HIGH_ACCURACY_FP16")

# Create/use the llm2clip_v3 collection
print("🔗 Setting up 'llm2clip_v3' collection...")
# Check if index exists, if not create it
if not faiss_db.is_trained:
    print("FAISS index does not exist. Creating...")
    faiss_db.addDatabase("llm2clip_v3", 1280, FEATURES_PATH, SPLIT_NAME)
else:
...
if faiss_db.is_trained:
    dummy_query_path = "/workspace/competitions/AIC_2025/SIU_Sayan/Base/Cache/dummy_query_llm2clip.npy"
    if os.path.exists(dummy_query_path):
        ...
        dummy_query = np.random.random(1280).astype('float32')
...
def preprocessing_text(text: str) -> np.ndarray:
    """Preprocess text to feature vector using LLM2Clip model"""
    global model
    ...
    # Handle different shapes: (1, D) or (D,)
    if text_feat_arr.ndim == 2 and text_feat_arr.shape[0] == 1:
        text_feat_arr = text_feat_arr[0]  # Flatten from (1, 1024) to (1024,)
    elif text_feat_arr.ndim == 1:
        pass  # Already correct shape (1024,)
    else:
        text_feat_arr = text_feat_arr.flatten()  # Fallback flatten
    
    return text_feat_arr.astype(np.float32)

def preprocessing_image(image: str) -> np.ndarray:
    """Preprocess image to feature vector using LLM2Clip model"""
    global model
    ...
    if image_feat_arr.ndim == 2 and image_feat_arr.shape[0] == 1:
        image_feat_arr = image_feat_arr[0]  # Flatten from (1, 1024) to (1024,)
    elif image_feat_arr.ndim == 1:
        pass  # Already correct shape (1024,)
    else:
        image_feat_arr = image_feat_arr.flatten()  # Fallback flatten
...
app = FastAPI(
    title="AIC 2025 SIU Sayan Video Search API - LLM2Clip (FAISS GPU)",
    description="Advanced video search API using LLM2Clip model with FAISS GPU vector database",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc",
    root_path="/siu_sayan_4"
)
...
async def index():
    ...
    return {
        "message": "AIC 2025 SIU Sayan Video Search API - LLM2Clip (FAISS GPU)",
        "version": "1.0.0",
        "endpoints": {
        ...
        "description": "Advanced video search using LLM2Clip model with FAISS GPU acceleration",
        "index_type": faiss_db.index_type,
        "feature_size": 1280
    }
...
if __name__ == "__main__":
    ...
    print(f"🚀 Starting LLM2Clip FAISS GPU API server on port 8512...")
