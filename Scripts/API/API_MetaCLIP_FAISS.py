...
from Extract_Feature.Class.MetaCLIP import MetaCLIP
...
print(f"✅ MetaCLIP model loaded in {model_load_time:.2f} seconds")

# Configuration
FEATURES_PATH = ["/dataset/AIC_2025/SIU_Sayan/autoshot/features_metaclip"]
SPLIT_NAME = ["autoshot"]

# Initialize FAISS GPU connection with HIGH_ACCURACY for best results
print("🔄 Initializing FAISS GPU connection...")
start_time = time.time()
faiss_db = FAISS_GPU("metaclip_v3", index_type="HIGH_ACCURACY")

# Create/use the metaclip_v3 collection
print("🔗 Setting up 'metaclip_v3' collection...")
# Check if index exists, if not create it
if not faiss_db.is_trained:
    print("FAISS index does not exist. Creating...")
    # MetaCLIP ViT-H/14 typically outputs 1024-d features
    faiss_db.addDatabase("metaclip_v3", 1024, FEATURES_PATH, SPLIT_NAME)
else:
...
if faiss_db.is_trained:
    dummy_query_path = "/workspace/competitions/AIC_2025/SIU_Sayan/Base/Cache/dummy_query_metaclip.npy"
...
def preprocessing_text(text: str) -> np.ndarray:
    """Preprocess text to feature vector using MetaCLIP model"""
    global model
...
def preprocessing_image(image: str) -> np.ndarray:
    """Preprocess image to feature vector using MetaCLIP model"""
    global model
...
app = FastAPI(
    title="AIC 2025 SIU Sayan Video Search API - MetaCLIP (FAISS GPU)",
    description="Advanced video search API using MetaCLIP model with FAISS GPU vector database",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc",
...
async def index():
    ...
    return {
        "message": "AIC 2025 SIU Sayan Video Search API - MetaCLIP (FAISS GPU)",
        "version": "1.0.0",
        "endpoints": {
        ...
        "description": "Advanced video search using MetaCLIP model with FAISS GPU acceleration",
        "index_type": faiss_db.index_type,
        "feature_size": 1024
    }
...
if __name__ == "__main__":
    ...
    print(f"🚀 Starting MetaCLIP FAISS GPU API server on port 8511...")
