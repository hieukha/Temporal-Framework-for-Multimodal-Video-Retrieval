...
from Extract_Feature.Class.SigLIP2 import SigLIP2
...
print(f"✅ SigLIP2 model loaded in {model_load_time:.2f} seconds")

# Configuration
FEATURES_PATH = ["/dataset/AIC_2025/SIU_Sayan/autoshot/features_siglip2"]
SPLIT_NAME = ["autoshot"]

# Initialize FAISS GPU connection with HIGH_ACCURACY for best results
print("🔄 Initializing FAISS GPU connection...")
start_time = time.time()
faiss_db = FAISS_GPU("siglip2_v3", index_type="HIGH_ACCURACY")

# Create/use the siglip2_v3 collection
print("🔗 Setting up 'siglip2_v3' collection...")
# Check if index exists, if not create it
if not faiss_db.is_trained:
    print("FAISS index does not exist. Creating...")
    faiss_db.addDatabase("siglip2_v3", 1536, FEATURES_PATH, SPLIT_NAME)
else:
...
if faiss_db.is_trained:
    dummy_query_path = "/workspace/competitions/AIC_2025/SIU_Sayan/Base/Cache/dummy_query_siglip2.npy"
    if os.path.exists(dummy_query_path):
        ...
        dummy_query = np.random.random(1536).astype('float32')
...
def preprocessing_text(text: str) -> np.ndarray:
    """Preprocess text to feature vector using SigLIP2 model"""
    global model
    ...
    # Handle both (1536,) and (1, 1536) shapes
    if text_feat_arr.ndim == 2 and text_feat_arr.shape[0] == 1:
        text_feat_arr = text_feat_arr[0]  # Flatten from (1, 1536) to (1536,)
    elif text_feat_arr.ndim == 1:
        pass  # Already correct shape (1536,)
    else:
...
def preprocessing_image(image: str) -> np.ndarray:
    """Preprocess image to feature vector using SigLIP2 model"""
    global model
    ...
    if image_feat_arr.ndim == 2 and image_feat_arr.shape[0] == 1:
        image_feat_arr = image_feat_arr[0]  # Flatten from (1, 1536) to (1536,)
...
app = FastAPI(
    title="AIC 2025 SIU Sayan Video Search API - SigLIP2 (FAISS GPU)",
    description="Advanced video search API using SigLIP2 model with FAISS GPU vector database",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc",
    root_path="/siu_sayan_3"
)
...
async def index():
    ...
    return {
        "message": "AIC 2025 SIU Sayan Video Search API - SigLIP2 (FAISS GPU)",
        "version": "1.0.0",
        "endpoints": {
        ...
        "description": "Advanced video search using SigLIP2 model with FAISS GPU acceleration",
        "index_type": faiss_db.index_type,
        "feature_size": 1536
    }
...
if __name__ == "__main__":
    ...
    print(f"🚀 Starting SigLIP2 FAISS GPU API server on port 8513...")
    uvicorn.run(app, host="0.0.0.0", port=8513)